...
global Tau
lmbd = 5
Tau = [0.25, 0.2, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0001]
...
if __name__ == "__main__":
    ...
    diffs = []
    for tau in Tau:
        times, vals = ig.approximation(func_e, \
                                       T=[0.0, 1.0], \
                                       X0=np.array([1.0]), \
                                       tau=tau)
        if not local:
    ...
    if local:
        ...
        plt.title("Implicit Gaus Local Error for lambda=" + str(lmbd))
    else:
        plt.ylabel("Global Error")
        plt.title("Implicit Gaus Global Error for lambda=" + str(lmbd))
    plt.grid()
    plt.show()
