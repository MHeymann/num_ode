...
import explicit_runge_kutta4 as erk
...
if __name__ == "__main__":
    ...
    #plt.plot(times, appx, label='Explicit RK4 Approximation')
    plt.plot(appx, appy, label='Explicit RK4 Approximation')

    plt.xlabel("X values")
    plt.ylabel("Y values")
    plt.title("Explicit RK4 Approximation for tau=" + str(Tau) + \
              ", k=" + str(k) + \
              ", (x0, y0)=(" + str(x0) + ", " + str(y0) + ")")
    plt.axis("equal")
    plt.show()
